...
@conf::path = ({
	    ...
	    },
	    {
	     ...
	     '
                foreach my $i (@items){
                    @auto_data=split(/\[__\]/,$i->{\'_content\'}[0]);
                    push(@rows,[@auto_data]);
                }
...
$conf::script="example2.php";  	      	      #Script file vulnerable
...
$conf::session="example2";  #Session name (You have to use the same name of the session config file without .pm extension)
